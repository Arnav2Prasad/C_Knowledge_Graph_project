...
3) Display array of choices : This is displayed as a "did you mean : " question as mentioned in (...)

% Depending on the user’s choice, the traversal is done.\\
...
\subsubsection{\texttt{display\_info\_lines}} :
...
\subsubsection{\texttt{print\_info\_lines}} :
    \begin{itemize}
        \item \textbf{Too Few lines allocated for printing} : 
...
In the worst case, the starting noun of traversal can be connected with $O(n)$ nodes. Hence, the worst-case time complexity of \texttt{print\_info\_lines} is
\begin{equation}
    T(n) = O(n\lg n)
\end{equation}
\end{itemize}

\texttt{display\_info\_lines} is also worst case $O(n \lg n)$, since its time complexity is asymptotically same.
